...
la_1seconde = np.arange(RATE)
x = np.arange(RATE)
plt.plot(x,la_1seconde)
```

```{code-cell} ipython3
# votre code
# la_1seconde = ...
"Le tableau (qui contient le LA de 1 seconde)  doit contenir 44100 valeurs."
"La fonction qui nous interesse est f(t) = sin(440/44100*2*np.pi*t)"
la_1seconde = np.sin(2 * np.pi * LA * (la_1seconde / RATE))
plt.plot(x[:500],la_1seconde[:500])
plt.show()
```
...
# votre code
def sine(freq, duration=1, amplitude=1.):
    #duratino doit être en secondes
    time = np.arange(RATE*duration)
    out = amplitude*np.sin(2 * np.pi * freq * (time/RATE))
    return out
```
...
def sine_linear(freq1, freq2, duration):
    ...
    out = np.sin(2*np.pi*freq*(time))
    return out
```

```{code-cell} ipython3
# décommenter pour écouter
sine_linear(440, 660, 3)
...
```{code-cell} ipython3
# votre code pour 1.
amplitudes = np.linspace(0,1,RATE)
crescendo_la_1seconde = la_1seconde * amplitudes
```

```{code-cell} ipython3
# décommenter pour écouter
MyAudio(crescendo_la_1seconde)
```
...
```{code-cell} ipython3
# votre code pour 2.
def crescendo_sine(freq, duration, amplitude=1):
    ampl = np.linspace(0,amplitude,RATE*duration)
    time = np.arange(RATE*duration)/RATE
    out = ampl * np.sin(2*np.pi*freq*time)
    return out
...
```{code-cell} ipython3
# votre code pour 3.
def crescendo_sine3(freq, duration, amplitude=1, increase=True):
    if increase:
        crescendo_sine(freq,duration,amplitude)
    else:
        ampl = np.linspace(amplitude,0,RATE*duration)
        return ampl*sine(freq,duration)
```

```{code-cell} ipython3
# décommenter pour écouter
MyAudio(crescendo_sine3(LA, 2, increase=False) , autoplay=True)
```
...
```{code-cell} ipython3
# votre code pour 4.
def crescendo(son,ampli_min=0,ampli_max=1,increase=True):
    duration = len(son)
    if increase:
        ampli = np.linspace(ampli_min,ampli_max,duration)
    else:
        ampli = np.linspace(ampli_max,ampli_min,duration)
    return ampli*son
```
...
# votre code
la_1seconde
la_do = sine(DO, duration=1, amplitude=1.)
la_do = np.concatenate((la_1seconde,la_do))
```
...
def float_to_int16(as_float):
    #je ne vérifie pas que les valeurs du tableau sont bien dans [-1,1]
    return (32767*as_float).astype(np.int16)
```
...
# décommenter pour écouter
t = float_to_int16(la_do)
print(t)
MyAudio(t, autoplay=True)
```
