...
def rectangle_size(n):
    main = int(np.sqrt(n)) + 1
    decr = main
    while ((main*(decr - 1)) > n):
        decr -= 1
    return (main,decr)

print(rectangle_size(8))
print(rectangle_size(11))
```
...
```{code-cell} ipython3
# votre code 
def patchwork(colors, width=10, background=[169, 169, 169]):
    """
    ...
    """
    n = len(colors)
    side,size = rectangle_size(n)
    print(n,side,size)

    Lin, Col = np.indices((size*width,side*width))
    ind = np.minimum((Lin//width)*side + (Col//width),n)
    temp = colors.copy()
...
```{code-cell} ipython3
plt.imshow(patchwork(colors+colors, width=10, background=[0, 0, 0]))
```
...
```{code-cell} ipython3
# votre code
# je code si il me reste du temps - je note juste le procédé
"""
ouverture et lecture d'un fichier en CSV et conversion sous la forme d'un tableau numpy
Extraction avec np.zeroes ou np.argwhere (si color est le nom de la couleur, np.argwhere(donnes[:0] = color)
Permet de faire le lien et de réutiliser la fonction patchwork précédente
...
def sepia_transform(img):
    sepib = np.array([[0.393, 0.769, 0.189],[0.349,0.686,0.168],[0.272,0.534,0.131]])
    sepi = sepib.T
    #print(sepi)
    out = np.dot(img,sepi)
    out2 = (np.minimum(out,255)).astype(np.uint8)
    return out2
```
...
```{code-cell} ipython3
# votre code
ins = plt.imread("data/les-mines.jpg")
#print(ins)
result = sepia_transform(ins)
#print(result)
plt.imshow(result)
#plt.imshow(ins)
```
